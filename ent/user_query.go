...
type UserQuery struct {
	...
	withOwnedNamespaces *NamespaceQuery
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withNamespaces; query != nil {
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "namespaces": %v`, err)
		}
	...
	if query := uq.withOwnedNamespaces; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[string]*User, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
		...
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(user.OwnedNamespacesPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, uq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "owned_namespaces": %v`, err)
		}
		query.Where(namespace.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
		...
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "owned_namespaces" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.OwnedNamespaces = append(nodes[i].Edges.OwnedNamespaces, n)
			}
		}
	}
